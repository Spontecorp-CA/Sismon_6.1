...
public class PerforacionEscenarioController2 extends SwingWorker<Map<Integer, Object[]>, Void> {
    ...
    private Map<Pozo, Map<String, Double[]>> tiemposPerforacionMap;
    ...
    public PerforacionEscenarioController2(Map<Integer, Object[]> estrategiaPerforacionMap,
    ...
    protected Map<Integer, Object[]> doInBackground() throws Exception {
        tiemposPerforacionMap = makePozoDataMap();
    ...
    private void procesarFila(FilaHasTaladro fhtSelected, boolean mudado) {
        ...
        Object[] elementos = new Object[14]; // arreglo de elemento a mostrar como resultado
        ...
        if (!statusTaladroMap.containsKey(taladroSelected)) {
            List<TaladroStatus> taladroStatusList = talStatusManager.findAll(taladroSelected,
                    Constantes.TALADRO_STATUS_OCUPADO);
            TaladroStatus taladroStatus = taladroStatusList.get(0);
...
//        TaladroHasFase thf = thfManager.find(fase, taladro, escenarioSelected);
        ...
        if(fechaIn.before(fechaCierre) || fechaIn.equals(fechaCierre) ){
            Map<String, Double[]> tiemposMap = tiemposPerforacionMap.get(pozo);
            dias = tiemposMap.get(fase)[0];
            items[11] = dias;
            items[12] = tiemposMap.get(fase)[1];
            items[13] = tiemposMap.get(fase)[2];
        } else {
            dias = thf.getDias();
            items[11] = dias;
            items[12] = dias;
        ...
        long diasLong = (long)dias;
        LocalDate ldOut = ldIn.plusDays(diasLong);
    ...
    private Date doEvaluacion(Date fechaIn, Object[] elementos) {
        ...
        if (fechaIn.before(fechaCierre) || fechaIn.equals(fechaCierre)) {
            ...
            items[11] = dias;
            items[12] = tiemposMap.get(Constantes.FASE_EVALUACION)[1];
            items[13] = tiemposMap.get(Constantes.FASE_EVALUACION)[2];
        } else {
    ...
    private Map<Pozo, Map<String, Double[]>> makePozoDataMap() {
        ...
        for (Perforacion perf : perforaciones) {
            ...
            if (pozoMap.containsKey(perf.getPozoId())) {
                ...
                Double[] dias = {perf.getDias(), perf.getDiasActivos(), perf.getDiasInactivos()}; 
                fasesMap.put(perf.getFase(), dias);
            } else {
